...
- **Гласов асистент** (изключен по подразбиране, отделен режим и своя клавишна комбинация) -
  кажете команда, асистентът предлага действие с глас и текст ("Да отворя ли Chrome?") и го
  изпълнява само след като кажете "да". В `v1` може само да отваря малък списък познати
  приложения или да търси нещо в Chrome - вижте [SECURITY.md](SECURITY.md) за модела на
  заплахите.
...
въвежда текст. Единственото изключение е отделният, изключен по подразбиране гласов асистент,
който изпълнява действие само след изрично гласово потвърждение - вижте по-горе.
...
| Гласов асистент | `Ctrl + Alt + K` (изключен по подразбиране) | да |
