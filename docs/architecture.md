...
клавишната комбинация трети път
  └─ записва отговора „да“/„не“
       └─ транскрипция → класификация                    core::assistant::classify_confirmation
            ├─ "да" → изпълнява действието                assistant::actions (ShellExecuteW)
            └─ всичко друго (включително тишина/неясно) → отказ, нищо не се изпълнява
```

AI моделът никога не се доверява пряко: той може само да избере id от статичен списък
(`core_assistant::KNOWN_APPS`), а Rust кодът винаги проверява отговора му отново, преди дори да
предложи действие на потребителя. Говоримият отговор минава през `src-tauri/src/tts.rs`
(вграденият в Windows WinRT `SpeechSynthesizer`, локално, без мрежа) и мълчаливо пада обратно
към само текст, ако няма инсталиран български глас.
