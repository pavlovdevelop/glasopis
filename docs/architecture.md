...
│                    гласов асистент: whitelist, потвърждение   │
└─────────────────────────────────────────────────────────────┘
```
...
## Потокът на гласовия асистент

Отделен, изключен по подразбиране режим (`src-tauri/src/assistant/`), със своя клавишна
комбинация и собствено пространство в настройките (`settings.assistant`). Никога не тече
едновременно с обикновената диктовка - двата дяла споделят записващото устройство
(`state.recorder`) и `speech::transcribe_with_settings`, но `assistant::cancel()` рестартира
асистентния etap, ако диктовка стартира междувременно.

```text
клавишна комбинация (Ctrl+Alt+K по подразбиране)
  └─ записва изречената команда
клавишната комбинация отново
  └─ спира записа → транскрипция (същият двигател като диктовката)
       └─ Groq chat completion, JSON отговор            speech::groq::interpret_command
       └─ отговорът се проверява спрямо fixed whitelist  core::assistant::parse_model_response
            ├─ непознат/halюциниран target → „Не разбрах командата.“, нищо не се изпълнява
            └─ познато действие → въпрос, изговорен и показан ("Да отворя ли Chrome?")
клавишната комбинация трети път
  └─ записва отговора „да“/„не“
       └─ транскрипция → класификация                    core::assistant::classify_confirmation
            ├─ "да" → изпълнява действието                assistant::actions
            └─ всичко друго (включително тишина/неясно) → отказ, нищо не се изпълнява
```

AI моделът никога не се доверява пряко: той може само да избере едно от два вида действие
(`core_assistant::ProposedAction`), а Rust кодът винаги проверява отговора му отново, преди дори
да предложи действие на потребителя:

- **`OpenApp`** - id от статичен списък (`core_assistant::KNOWN_APPS`), стартирано през
  `ShellExecuteW`.
- **`SearchChrome`** - свободен текст за търсене, но той единствено се вкарва в адресната лента
  на Chrome (`injection::focus::activate_process_window` намира/активира прозореца му,
  `injection::keyboard::send_focus_address_bar` изпраща `Ctrl+L`, текстът се въвежда като
  обикновена диктовка, после `Enter`) - никога произволни клавишни комбинации или кликове.

Говоримият отговор минава през `src-tauri/src/tts.rs` (вграденият в Windows WinRT
`SpeechSynthesizer`, локално, без мрежа) и мълчаливо пада обратно към само текст, ако няма
инсталиран български глас.
...
| `glasopis-assistant-command` | транскрипция на командата + AI тълкуване (изключен по подразбиране) |
| `glasopis-assistant-confirm` | транскрипция на „да“/„не“ + изпълнение при потвърждение |
