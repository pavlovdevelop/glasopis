...
- **Без телеметрия**, историята е изключена по подразбиране, аудиото остава само в паметта.
...
3. При първото стартиране помощникът ще ви преведе през избор на микрофон, въвеждане на
   безплатен API ключ от [console.groq.com/keys](https://console.groq.com/keys) и клавишната
   комбинация.
...
В режима по подразбиране записът се изпраща към сървърите на Groq за разпознаване и се обработва
там според техните условия. Glasopis не записва аудио на диска — то съществува само в паметта.
В локална компилация нищо не напуска компютъра. Историята на диктовките е изключена по
подразбиране. Подробности: [docs/privacy.md](docs/privacy.md).
