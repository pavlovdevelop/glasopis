...
mod tests {
    ...
    fn new_line_and_paragraph() {
        assert_eq!(
            process("първи ред нов ред втори ред", true),
            "първи ред\nвтори ред"
        );
        assert_eq!(
    ...
    fn delete_commands() {
        assert_eq!(
            process("едно две изтрий последната дума три", true),
            "едно три"
        );
        assert_eq!(
            process(
                "Първо изречение точка Второ изречение точка изтрий последното изречение",
                true
            ),
            "Първо изречение."
        );
    ...
    fn commands_can_be_disabled() {
        assert_eq!(
            process("Здравей запетая как си", false),
            "Здравей запетая как си"
        );
    }
