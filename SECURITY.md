...
- **Гласовият асистент** (изключен по подразбиране, отделен от диктовката). AI моделът, който
  разбира командата, никога не се доверява пряко - той може само да избере id от фиксиран,
  вграден в кода списък с приложения; ако предложи нещо извън него, Rust кодът го отхвърля и
  нищо не се изпълнява. Дори при валидно предложение асистентът пита на глас и с текст ("Да
  отворя ли Chrome?") и изпълнява действието само след като разпознае гласово "да" - всеки друг
  отговор, тишина или неясен отговор отменя действието. В `v1` единственото възможно действие е
  стартиране на едно от малък списък познати приложения (напр. Chrome, Бележник) - не и
  произволно изпълнение на команди.
