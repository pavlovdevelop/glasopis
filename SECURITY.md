...
- **Гласовият асистент** (изключен по подразбиране, отделен от диктовката). AI моделът, който
  разбира командата, никога не се доверява пряко - той може само да избере едно от фиксиран,
  вграден в кода списък от действия (не свободен текст, не команди); ако предложи нещо извън тях,
  Rust кодът го отхвърля и нищо не се изпълнява. Дори при валидно предложение асистентът пита на
  глас и с текст ("Да отворя ли Chrome?") и изпълнява действието само след като разпознае гласово
  "да" - всеки друг отговор, тишина или неясен отговор отменя действието. В `v1` възможните
  действия са две, и двете тесни и одитируеми: стартиране на едно от малък списък познати
  приложения (напр. Chrome, Бележник), или търсене в Chrome - модел избира *текста за търсене*,
  но само той се вкарва в адресната лента на браузъра (`Ctrl+L` + текст + Enter) - никога
  произволни клавишни комбинации, кликове или изпълнение на команди.

Какво умишлено не прави:

- Glasopis никога не изпълнява разпознатия текст като команда и никога не натиска Enter вместо
  вас. Гласовият асистент по-горе е единственото, умишлено ограничено изключение от това правило:
  той никога не изпълнява свободен текст, а само избор от фиксиран списък, потвърден гласово от
  вас.
